...
let links = ["https://i.ibb.co/4nRyMK6p/image.png"];
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.main = function(ev) {
    ...
    rangeInputs.forEach(input => {
      input.addEventListener('input', function() {
        this.style.background = `linear-gradient(90deg, var(--accent-color) ${this.value / this.max * 100}%, var(--light-accent) ${this.value / this.max * 100}%)`;
      });
  ...
  window.analyzeImage = async function(img) {
    try {
      await originalAnalyzeImage(img);
    } finally {
      loadingOverlay.classList.remove('active');
    }
  };
  
  const originalEnableInputs = window.enableInputs;
  window.enableInputs = function(enabled) {
    originalEnableInputs(enabled);
    ...
    if (enabled) {
      ...
      resultContainer.classList.add('result-loaded');
    } else {
      controls.classList.remove('controls-enabled');
      resultContainer.classList.remove('result-loaded');
    }
  };
  ...
  const settingsColumns = document.querySelectorAll('.settings-col');
  settingsColumns.forEach(col => {
    col.addEventListener('mouseenter', function() {
      this.classList.add('settings-col-hover');
    });
    col.addEventListener('mouseleave', function() {
      this.classList.remove('settings-col-hover');
    });
  });
